...
const mainServer = import.meta.env.VITE_SERVER_MAIN_HOST
const socket = io.connect(`${mainServer}`)
...
const trackDriver = async (e)=>
{
  ...
  const setupDarkMap = (lng, lat) => {
    ...
    marker.current = new mapboxgl.Marker({
      element: markerTrack.current, scale: '0'
    }).setPopup(new mapboxgl.Popup().setHTML("<p>Driver is Here!</p>")) // add popup
  ...
  const getVehicleStats = async () => {
    try {
      const result = await axios.get(`${hostServer}/get-vehicle-stats/${tripDetail?.t_id}`)
      const data = result.data
      console.log(data[0])
      setVehicleStats(data[0])
    } catch (error) {
      console.log(error)
    }
  ...
  const getDriverPosition = async () => {
    try {
      const drivePosition = await axios.get(`${hostServer}/getPosition/${tripDetail?.t_id}`)
  ...
  useEffect(() => {
    socket.on('deliveryUpdate', async (data) => {
      ...
      if(tripData?.t_id == trip_id){
        alert(`Delivery Status: ${deliveryState}`)
...
/>
       ...
       {isFormNotSubmitted && 
       (<>
              <div className="trackingBackground"></div>
       <div className="trackingContainer">
        ...
        <div className="trackingFooter">
          ...
          <p>Copyright © 2024, Kargada. All Rights Reserved.</p>
          </div>
